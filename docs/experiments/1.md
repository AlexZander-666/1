...
| 论文名称 | 代码对应 | 架构组成 | 说明 |
|----------|----------|----------|------|
| **PhyCL-Net** (提出模型) | `--model phycl_net` | FAA + PDK | 纯时域轻量架构，移除MSPA频谱分支 |
| **MSPA-FAA-PDK** (基线对照) | `--model mspa_faa_pdk` | MSPA + FAA + PDK | 完整三模块架构，含频谱分支 |
...
| 论文名称 | 代码类名 | 配置参数 | 说明 |
|----------|----------|----------|------|
| **PhyCL-Net** (提出模型) | `PhyCL_Net` | `use_mspa=False` | 纯时域架构，保留FAA+PDK |
| **MSPA-FAA-PDK** (基线对照) | `PhyCL_Net` | `use_mspa=True` | 完整三模块架构 |

**训练脚本配置**:
```bash
# 主模型 (PhyCL-Net) - 移除MSPA频谱分支
python code1/phycl_net_experiments.py --model phycl_net

# 基线对照 (MSPA-FAA-PDK) - 完整三模块架构
python code1/phycl_net_experiments.py --model mspa_faa_pdk
```
...
#### CrossGatedFusion - `models/PhyCL_Net.py`
...
#### 完整5种子LOSO验证
```bash
python code1/phycl_net_experiments.py --dataset sisfall --data-root ./data --model phycl_net \
  --eval-mode loso --seeds 42 123 456 789 1024 --epochs 50 --batch-size 256 --lr 0.004 --warmup-epochs 10 \
...
```bash
python code1/phycl_net_experiments.py --dataset sisfall --data-root ./data --model phycl_net \
  --eval-mode loso --seeds 42 123 --epochs 50 --batch-size 256 --lr 0.004 --warmup-epochs 10 \
  --amp --weighted-loss --use-tfcl --ablation dks:False --out-dir ./outputs/ablation_no_dks
```
...
python code1/phycl_net_experiments.py --dataset sisfall --data-root ./data --model phycl_net \
  --eval-mode loso --seeds 42 123 --epochs 50 --batch-size 256 --lr 0.004 --warmup-epochs 10 \
  --amp --weighted-loss --use-tfcl --ablation faa:False --out-dir ./outputs/ablation_no_faa
```

#### 基线模型
```bash
# LSTM
python code1/phycl_net_experiments.py --dataset sisfall --model lstm \
  --eval-mode loso --seeds 42 123 456 --epochs 100 \
  --out-dir ./outputs/baseline_lstm

# ResNet
python code1/phycl_net_experiments.py --dataset sisfall --model resnet \
  --eval-mode loso --seeds 42 123 456 --epochs 100 \
  --out-dir ./outputs/baseline_resnet

# TCN
python code1/phycl_net_experiments.py --dataset sisfall --model tcn \
  --eval-mode loso --seeds 42 123 456 --epochs 100 \
  --out-dir ./outputs/baseline_tcn

# Transformer
python code1/phycl_net_experiments.py --dataset sisfall --model transformer \
  --eval-mode loso --seeds 42 123 456 --epochs 100 \
  --out-dir ./outputs/baseline_transformer

# InceptionTime
python code1/phycl_net_experiments.py --dataset sisfall --model inceptiontime \
  --eval-mode loso --seeds 42 123 456 --epochs 100 \
...
| 实验 | 结果文件路径 |
|------|-------------|
| MSPA-FAA-PDK (基线) | `outputs/mspa_faa_pdk_baseline/summary_results.json` |
| InceptionTime基线 | `outputs/stage1_inceptiontime_final/summary_results.json` |
| Transformer基线 | `outputs/stage1_transformer_final/summary_results.json` |
| w/o TFCL | `outputs/ablation_no_tfcl/summary_results.json` |
| w/o MSPA | `outputs/phycl_net/summary_results.json` |
| w/o DKS | `outputs/ablation_no_dks/summary_results.json` |
| w/o FAA | `outputs/ablation_no_faa/summary_results.json` |
| Time-only | `outputs/ablation_time_only/summary_results.json` |
| Freq-only | `outputs/ablation_freq_only/summary_results.json` |

### A.1 输出目录结构

```
outputs/
├── mspa_faa_pdk_baseline/          # MSPA-FAA-PDK（谱域 baseline）
├── phycl_net/                     # PhyCL-Net（无MSPA）
...
| 模块 | 文件路径 | 说明 |
|------|----------|------|
| 主训练脚本 | `code1/phycl_net_experiments.py` | 包含所有实验配置 |
| **PhyCL-Net (提出)** | `code/models/PhyCL_Net.py` | 设置 `use_mspa=False` |
| **MSPA-FAA-PDK (基线)** | `code/models/PhyCL_Net.py` | 设置 `use_mspa=True` |
| PDK模块 | `code/models/modules/dks.py` | 物理引导动态核选择 |
| FAA模块 | `code/models/modules/faa.py` | 跌倒感知注意力 |
| TFCL损失 | `code/losses/tfcl.py` | 时频对比学习 |
| 数据集加载 | `code1/phycl_net_experiments.py` | SisFall预处理 |

**代码-论文命名映射**: 
| 论文名称 | 代码实现 | 配置参数 |
|----------|----------|----------|
| **PhyCL-Net** (提出模型) | `PhyCL_Net` | `use_mspa=False` |
| **MSPA-FAA-PDK** (基线对照) | `PhyCL_Net` | `use_mspa=True` |

- 所有实验结果可通过 `code1/phycl_net_experiments.py` 复现（`--model phycl_net` 为 no-MSPA；`--model mspa_faa_pdk` 为谱域 baseline；其他消融使用 `--ablation ...`）
...
| 论文中的名称 | 代码实现 | 配置参数 |
|-------------|---------|---------|
| **PhyCL-Net (提出模型)** | `models/PhyCL_Net.py::PhyCL_Net` | `use_mspa=False` |
| **MSPA-FAA-PDK (基线对照)** | `models/PhyCL_Net.py::PhyCL_Net` | `use_mspa=True` |
| **Training Script** | `code1/phycl_net_experiments.py` | `--model phycl_net` / `--model mspa_faa_pdk` |
...
```bash
  ...
  python code1/phycl_net_experiments.py --dataset sisfall --data-root ./data --model phycl_net \
    --eval-mode loso --seeds 42 123 456 789 1024 --epochs 50 --batch-size 256 --lr 0.004 --warmup-epochs 10 \
    --amp --weighted-loss --use-tfcl --out-dir ./outputs/phycl_net

  # 基线对照 (MSPA-FAA-PDK)
  python code1/phycl_net_experiments.py --dataset sisfall --data-root ./data --model mspa_faa_pdk \
    --eval-mode loso --seeds 42 123 --epochs 50 --batch-size 256 --lr 0.004 --warmup-epochs 10 \
    --amp --weighted-loss --use-tfcl --out-dir ./outputs/mspa_faa_pdk_baseline
...
- [ ] 代码-论文命名映射已在补充材料中说明:
  - PhyCL-Net → `--model phycl_net`
  - MSPA-FAA-PDK → `--model mspa_faa_pdk`
- [ ] 训练脚本配置参数已记录
