...
python code1/phycl_net_experiments.py --model amsv2 --ablation mspa:True
```
...
#### 完整5种子LOSO验证
```bash
python code1/phycl_net_experiments.py --dataset sisfall --model phycl_net \
...
#### 消融: w/o DKS
```bash
python code1/phycl_net_experiments.py --dataset sisfall --model amsv2 \
...
- 所有实验结果可通过 `code1/phycl_net_experiments.py` 复现（`--model phycl_net` 为 no-MSPA；`--model amsv2 --ablation mspa:True/False` 为频谱分支开关）
...
| **Training Script** | `code1/phycl_net_experiments.py` | `--model phycl_net` / `--model amsv2 --ablation mspa:True/False` |
