...
class Phycl_NetBlock(nn.Module):
    """
    Phycl_Net block combining dynamic kernels, multi-scale spectral modeling,
...
class Phycl_Net(nn.Module):
    """
    Phycl_Net: Physics-Inspired Contrastive Lightweight Network with lightweight stem,
    Phycl_Net blocks, projection head for contrastive learning, and classifier head.
    """
    ...
    def __init__(
        ...
        super().__init__()
        # PhyCL-Net (paper main model) removes MSPA by default; MSPA can be
        # re-enabled via ablation config for the spectral baseline.
        ablation = ablation or {"mspa": False, "dks": True, "faa": True}
        stem_channels = 48
    ...
    def forward(
        ...
        # TFCL is training-only (per paper). By default, avoid emitting/creating
        # embeddings in eval/inference unless explicitly requested.
        if return_embeddings is None:
            return_embeddings = self.training
