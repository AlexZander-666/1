...
此脚本实现了 PhyCL-Net (Physics-Inspired Contrastive Lightweight Network) 的训练与评估流程，
并提供多种对照基线（DMC/ResNet/TCN/Transformer/InceptionTime 等）。
...
Usage:
    # 快速验证 (Dry Run)
    python PhyCL_Net_experiments.py --dataset dryrun --profile --epochs 2 --batch-size 4

    # 主模型 (PhyCL-Net, No MSPA) - 训练
    python PhyCL_Net_experiments.py --dataset sisfall --data-root ./data --model phycl_net --eval-mode loso --seeds 42 123 --epochs 100 --batch-size 16 --weighted-loss --amp --use-tfcl --ablation mspa:False

    # 真实实验 - 断点续训
    python PhyCL_Net_experiments.py --dataset sisfall --data-root ./data --model phycl_net --resume ./outputs/ckpt_last_seed42.pth
"""
...
from losses import Phycl_NetLoss
...
def parse_ablation_config(spec: Optional[str]) -> Dict[str, bool]:
    ...
    if isinstance(spec, dict):
        return spec
    if not spec:
        # Empty spec means: use each model's own defaults.
        return {}
    key = str(spec).lower()
    if key in ABLATION_PRESETS:
        return dict(ABLATION_PRESETS[key])

    allowed = {'mspa', 'dks', 'faa', 'tfcl', 'center'}
    ...
    if spec and not config:
        logging.warning("ablation spec unrecognized; ignoring")
    return config
...
def compute_class_weights(dataset, num_classes=2, eps=1e-6, max_weight=10.0):
    """
    [Patch C] 增加了 max_weight clipping，防止极度不平衡导致梯度爆炸。
    """
    try:
...
def compute_loss_with_aux(criterion, logits, targets, z_time=None, z_freq=None):
    """
    Compute loss and auxiliary components, supporting Phycl_NetLoss as well as standard criterions.
    """
    if isinstance(criterion, Phycl_NetLoss):
...
def _select_gradcam_layer(model: nn.Module) -> Optional[nn.Module]:
    # Prefer last Phycl_Net block if available
...
def run_one_experiment(config, seed, resume_path=None):
    ...
    def train_eval_split(train_ds, val_ds, test_ds, split_tag: str, resume_path_split: Optional[str]):
        ...
        elif config['model'] == 'lite_phycl_net':
            model = LitePhycl_Net(
                in_channels=C,
                num_classes=config['num_classes'],
        ...
        scaler = torch_amp.GradScaler(device=AMP_DEVICE_TYPE) if config['amp'] and device.type == 'cuda' else None
...
def main():
    ...
    p.add_argument('--attn-lite', type=str, default='none', choices=attn_choices, help="Attention inside LitePhycl_Net blocks")
    ...
    p.add_argument('--loss-alpha', type=float, default=0.1, help="Weight for TF contrastive loss term")
    p.add_argument('--loss-beta', type=float, default=0.01, help="Weight for center loss term")
